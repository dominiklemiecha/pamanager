...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($postAction) {
        case 'create':
            ...
            if ($result['success']) {
                header('Location: consulente-lavoro.php?message=created');
                exit;
            }
            $error = $result['error'];
            $action = 'new';
        ...
        case 'update':
            if ($id) {
                ...
                if ($result['success']) {
                    header('Location: consulente-lavoro.php?message=updated');
                    exit;
                }
                $error = $result['error'];
                $action = 'edit';
        ...
        case 'delete':
            if ($id) {
                ...
                if ($result['success']) {
                    header('Location: consulente-lavoro.php?message=deleted');
                    exit;
                }
                $error = $result['error'];
            }
...
if (isset($_GET['message'])) {
    $messages = [
        'created' => 'Consulente del lavoro creato con successo',
        'updated' => 'Consulente del lavoro aggiornato con successo',
        'deleted' => 'Consulente del lavoro eliminato con successo'
    ];
...
if ($action === 'list') {
    $accountants = User::getConsulentiLavoro();
} elseif (($action === 'edit' || $action === 'view') && $id) {
    $accountant = User::getById($id);
    if (!$accountant || $accountant['role'] !== 'consulente_lavoro') {
        header('Location: consulente-lavoro.php?error=not_found');
        exit;
    }
}

$pageTitle = $action === 'new' ? 'Nuovo Consulente del lavoro'
           : ($action === 'edit' ? 'Modifica Consulente del lavoro'
...
<div class="admin-page">
    <div class="page-header">
        ...
        <?php if ($action === 'list'): ?>
            <a href="?action=new" class="btn btn-primary">Nuovo Consulente del lavoro</a>
        <?php else: ?>
            <a href="consulente-lavoro.php" class="btn btn-secondary">Torna alla Lista</a>
        <?php endif; ?>
    </div>
        ...
        <form method="POST" class="form-card">
            ...
            <div class="form-actions">
                <button type="submit" class="btn btn-primary">
                    <?= $action === 'new' ? 'Crea Consulente del lavoro' : 'Salva Modifiche' ?>
                </button>
                <a href="consulente-lavoro.php" class="btn btn-secondary">Annulla</a>
            </div>
        </form>
