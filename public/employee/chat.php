...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ob_start();
    // Disabilita display errori per evitare output HTML
...
if (!class_exists('Notification')) {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        header('Content-Type: application/json');
        echo json_encode(['success' => false, 'error' => 'Classe Notification non trovata.']);
        exit;
    }
    die('Errore: Classe Notification non trovata. Verificare che src/classes/Notification.php sia stato caricato sul server.');
}
...
.chat-empty-state { padding: 3rem 1.5rem; text-align: center; color: #a0aec0; }
        ...
        ?>
            <div class="chat-header">
                ...
                <div class="chat-header-info">
                    ...
                    <div class="chat-header-status"><?= ucfirst(str_replace('_', ' ', $selectedConv['other_type'])) ?></div>
                </div>
            </div>
...
<div class="modal-overlay" id="newChatModal">
    <div class="modal">
        ...
        <?php if (!empty($contacts['accountant'])): ?>
            <div class="contact-group">
                <div class="contact-group-title">Commercialisti</div>
                <div class="contact-list">
                    <?php foreach ($contacts['accountant'] as $c): ?>
                        <div class="contact-item" onclick="startConversation('accountant', <?= $c['id'] ?>)">
        ...
        <?php if (!empty($contacts['admin_reparto'])): ?>
            <div class="contact-group">
                <div class="contact-group-title">Admin del tuo Reparto</div>
                <div class="contact-list">
        ...
        <?php if (empty($contacts['accountant']) && empty($contacts['admin_reparto'])): ?>
            <p style="color: #a0aec0; text-align: center;">Nessun contatto disponibile</p>
        <?php endif; ?>
...
document.getElementById('newChatModal').addEventListener('click', function(e) {
    if (e.target === this) hideNewChatModal();
});
</script>
